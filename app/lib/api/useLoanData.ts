...
interface UseLoanDataParams {
  initialLoan?: any;
  onSubmit: (loan: any) => void; //FormData -> any
}

// This function is for adminUser only
export function useLoanData({ initialLoan, onSubmit }: UseLoanDataParams) {
  ...
  useEffect(() => {
    if (initialLoan) {
      setTitle(initialLoan.title || "");
      setIsbn(initialLoan.isbn || "");
      setAuthorId(initialLoan.authorId || "");
      setAuthorName(initialLoan.authorName || initialLoan.author?.name || "");
      setCategoryId(initialLoan.categoryId || "");
      setPublishedYear(initialLoan.publishedYear || "");
      setDescription(initialLoan.description || "");
      setPreviewUrl(initialLoan.coverImage || "");
      setTotalCopies(initialLoan.totalCopies || "1");
      setAvailableCopies(initialLoan.availableCopies || "1");

      if (initialLoan.coverImage) {
        setPreviewUrl(initialLoan.coverImage);
      } else {
        setPreviewUrl("");
      }
